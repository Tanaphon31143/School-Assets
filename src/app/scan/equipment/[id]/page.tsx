...
export default function PublicEquipment() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.card}>
      <div className={styles.hero}><div className={styles.image}>{typeof item.image_url === "string" && item.image_url ? <img src={item.image_url} alt={`รูปครุภัณฑ์ ${String(item.name)}`} /> : <span>▣</span>}</div><div className={styles.title}><small>{String(item.code)}</small><h2>{String(item.name)}</h2><em className={styles.status}>{status[String(item.status)] ?? String(item.status)}</em></div></div>
