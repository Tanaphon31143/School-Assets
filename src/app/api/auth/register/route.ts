...
export async function POST(request: Request) {
  ...
  const [result] = await db.query(
    ...
    [name, email, await bcrypt.hash(body.password, 10), "active"],
  );
  const userId = (result as { insertId: number }).insertId;
  const [roleRows] = await db.query("SELECT id FROM roles WHERE name = ? LIMIT 1", ["student"]);
  const studentRole = (roleRows as { id: number }[])[0];

  if (studentRole) {
    await db.query(
      "INSERT INTO model_has_roles(role_id,model_type,model_id) VALUES(?,?,?)",
      [studentRole.id, "App\\Models\\User", userId],
    );
  }
