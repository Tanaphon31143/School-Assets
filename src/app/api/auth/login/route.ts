...
export async function POST(request: Request) {
  try {
    ...
    const [rows] = await db.query("SELECT u.id, u.name, u.email, u.password, u.status, COALESCE(r.name, 'user') AS role FROM users u LEFT JOIN model_has_roles m ON m.model_id = u.id AND m.model_type LIKE '%User' LEFT JOIN roles r ON r.id = m.role_id WHERE u.email = ? LIMIT 1", [email]);
    console.log("[Login] DB query success, rows found:", (rows as []).length);

    const user = (rows as { id: number; name: string; email: string; password: string; status: string; role: string }[])[0];
    if (!user || user.status === "deleted" || !(await bcrypt.compare(password, user.password))) return NextResponse.json({ error: "อีเมลหรือรหัสผ่านไม่ถูกต้อง" }, { status: 401 });
