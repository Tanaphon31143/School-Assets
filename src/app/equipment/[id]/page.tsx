...
export default function EditEquipment() {
  ...
  if (!item)
    return <main className={styles.page}><p>กำลังโหลดข้อมูล...</p>
    </main>;
      ...
      <div className={styles.grid}>
        ...
        <label>ประเภท
          <select name="equipment_category_id" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} required>
            <option value="">เลือกประเภท</option>{categories.map((category) => <option key={String(category.id)} value={String(category.id)}>{String(category.name)}</option>)}
        ...
        <label>สถานที่
          <select name="equipment_location_id" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)}>
            ...
            {locations.map((location) => <option key={String(location.id)} value={String(location.id)}>{String(location.name)}</option>)}
            <option value="custom">เพิ่มเติม...</option>
          </select>
        </label>
        {selectedLocation === "custom" &&
          <label>ระบุสถานที่เพิ่มเติม<input name="custom_location" placeholder="กรอกชื่อสถานที่ เช่น ห้อง 301, อาคาร 2" required autoFocus /></label>}
        <label>ยี่ห้อ<input name="brand" defaultValue={String(item.brand ?? "")} /></label>
        <label>รุ่น<input name="model" defaultValue={String(item.model ?? "")} /></label>
        <label>Serial Number<input name="serial_number" defaultValue={String(item.serial_number ?? "")} /></label>
        <label>วันที่ซื้อ<input type="date" name="purchase_date" defaultValue={String(item.purchase_date ?? "").slice(0, 10)} /></label>
        <label>ราคาซื้อ<input type="number" step="0.01" name="purchase_price" defaultValue={String(item.purchase_price ?? "")} /></label>
        <label>จำนวน<input type="number" min="1" name="quantity" defaultValue={String(item.quantity ?? 1)} /></label>
        <label>หน่วย<input name="unit" defaultValue={String(item.unit ?? "ชิ้น")} /></label>
        <label>สถานะ<select name="status" defaultValue={String(item.status ?? "available")}>
        ...
        <label>สภาพ<select name="condition" defaultValue={String(item.condition ?? "good")}>
          <option value="good">good</option>
          <option value="fair">fair</option>
          <option value="poor">poor</option>
        </select>
        </label>
        <label>หมดประกัน<input type="date" name="warranty_expiry" defaultValue="" /></label>
      </div>
